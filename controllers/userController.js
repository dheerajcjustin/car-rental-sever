...
exports.bookings = async (req, res) => {
      try {
            ...
            let completedEvents = await Order.aggregate([
                  {
                        $match: {
                              $and: [
                                    { userId: userId },
            ...
            let upComingEvents = await Order.aggregate([
                  { $match: { $and: [{ userId }, { dropOffStatus: false }] } },
                  {
                        $lookup: {
                              from: "cars",
                              localField: "carId",
...
exports.reportVendor = tryCatch(async (req, res) => {
      ...
      if (!message || !carId || !vendorId)
            return res
                  .status(400)
                  .json("invalid data ,must have 'message','carId','vendorId'");
...
exports.patchProfilePic = tryCatch(async (req, res) => {
      ...
      const profile = await User.findByIdAndUpdate(vendorId, {
