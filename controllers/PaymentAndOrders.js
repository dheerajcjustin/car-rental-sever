...
exports.paymentDone = async (req, res) => {
      ...
      if (!carId || !payAmount || !selectedPickup || !bookingTime)
      ...
      const selectLocation = await Location.findOne({
            "pickupPoints._id": pickup,
      });
      const pickupPoint = selectLocation.pickupPoints.filter(
            (pick) => pick.id == pickup,
      );
      ...
      const order = new Order({
            ...
            price: payAmount,
            selectedPickup,
            pickupDate: bookingTime.pickupDate,
            dropOffDate: bookingTime.dropOffDate,
            pickupTime: bookingTime.pickupTime.trim(),
            dropOffTime: bookingTime.dropOffTime.trim(),
            pickup: pickupPoint[0],
            userId,
      });
      console.log(order);
      await order.save();
