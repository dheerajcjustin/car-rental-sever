      ...
      try {
            ...
            if (
                  ...
                  return res.status(400).json({
                        ...
                        example: "/search?pickupDate=${searchOptions.pickupDate}&pickupTime=${pickupTime}&dropOffDate=${searchOptions.dropOffDate}&dropOffTime=${searchOptions.dropOffTime} &locationId=${locationId}",
                  });
            ...
            if (sort) {
                  if (sort.trim() == "high") {
                        cars = await Car.find({ location: locationId }).sort({
                              price: -1,
                        });
                  }
                  if (sort.trim() == "low") {
            ...
            const pickups = availablePickups.map(
                  (data) => data.pickupPointDetails[0].pickupPoints,
            )[0];
...
const home = async (req, res) => {
      try {
            const locations = await Location.find({
                  pickupPoints: { $exists: true, $type: "array", $ne: [] },
            }).select("location description image pickupPoints");
